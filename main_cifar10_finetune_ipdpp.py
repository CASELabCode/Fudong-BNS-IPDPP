...
from datasets.imbalance_cifar import ImbalanceCIFAR10

from sampling.ip_dpp_smapler import IPDPPSampler
...
def get_args_parser():
    parser = argparse.ArgumentParser('IP-DPP Fine-tuning', add_help=False)
    ...
    parser.add_argument('--sampling_freq', default=10, type=int, help='frequency of sampling')
    parser.add_argument('--num_classes', default=10, type=int, help='number of classes')
    parser.add_argument('--num_samples', default=500, type=int, help='number of samples per class')

    parser.add_argument('--resume', default='', help='resume from checkpoint')
    parser.add_argument('--eval', default='', help='checkpoint to evaluate')

    # dataset
    parser.add_argument('-dr', '--data_root', type=str, default='/mnt/data/cfiar10')
    parser.add_argument('-sf', '--save_freq', type=int, default=1, help='checkpoint save frequency')
    ...
    parser.add_argument('--output_dir', default='./output_dir/cifar10/finetune_ipdpp_cifar10',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_dir/cifar10/finetune_ipdpp_cifar10',
...
def main(args):
    ...
    original_train_set = ImbalanceCIFAR10(root=args.data_root, imb_factor=1/args.imbalanced_factor, train=True, transform=train_transform, download=True)
    test_set = CIFAR10(root=args.data_root, train=False, transform=test_transform, download=False)

    # Compute class counts and categorize classes
    class_counts = compute_class_counts(original_train_set)
    many_shot_classes, medium_shot_classes, few_shot_classes = categorize_classes(class_counts)
    ...
    base_encoder = models_resnet.ResNet18(num_classes=args.num_classes)
    ...
    if args.eval:
        ...
        many_shot_acc, medium_shot_acc, few_shot_acc = evaluate_model(model, test_loader, many_shot_classes, medium_shot_classes, few_shot_classes)
        print(
    ...
    num_samples = [args.num_samples] * args.num_classes
    sampler = IPDPPSampler(original_train_set.data, np.array(original_train_set.targets), class_num=args.num_classes,
                           batch_size=args.batch_size,
                           transform=train_transform)
    train_set = sampler(model, num_samples)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.sampling_freq == 0 and (epoch + 1) != args.epochs:
            train_set = sampler(model, num_samples)
            train_loader = torch.utils.data.DataLoader(
                train_set,
                batch_size=args.batch_size,
